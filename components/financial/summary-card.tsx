...
import { TrendingUp, TrendingDown, DollarSign, Wallet, PieChart, AlertCircle, CheckCircle2 } from "lucide-react";
...
export function SummaryCard({ summary, currency }: SummaryCardProps) {
  ...
  const porcentajeGastosFijos = totalIngresos > 0 ? (totalGastosFijos / totalIngresos) * 100 : 0;
  const porcentajeGastosVariables = totalIngresos > 0 ? (totalGastosVariables / totalIngresos) * 100 : 0;
  ...
  // Estado financiero
  const estadoFinanciero = balance > totalIngresos * 0.2 
    ? { estado: "Excelente", color: "text-green-600 dark:text-green-400", icon: CheckCircle2 }
    : balance > 0 
    ? { estado: "Saludable", color: "text-blue-600 dark:text-blue-400", icon: TrendingUp }
    : { estado: "Crítico", color: "text-red-600 dark:text-red-400", icon: AlertCircle };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Resumen completo de tu situación financiera</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-muted-foreground">Estado</p>
          <p className={`text-xl font-bold flex items-center gap-2 ${estadoFinanciero.color}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* Total Gastos */}
        <Card className="border-red-500/20 bg-linear-to-br from-red-500/5 to-transparent animate-in fade-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: '50ms' }}>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-muted-foreground">
                Total Gastos
        ...
        {/* Balance */}
        <Card className={`${isPositive ? 'border-green-500/20 bg-linear-to-br from-green-500/5' : 'border-orange-500/20 bg-linear-to-br from-orange-500/5'} to-transparent animate-in fade-in slide-in-from-bottom-2 duration-300`} style={{ animationDelay: '100ms' }}>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-muted-foreground">
                Balance Final
              </CardTitle>
              <div className={`h-8 w-8 rounded-lg ${isPositive ? 'bg-green-500/10' : 'bg-orange-500/10'} flex items-center justify-center`}>
                <Wallet className={`h-4 w-4 ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`} />
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <p className={`text-2xl font-bold ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>
        ...
        {/* Tasa de Gasto */}
        <Card className="border-blue-500/20 bg-linear-to-br from-blue-500/5 to-transparent animate-in fade-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: '150ms' }}>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-muted-foreground">
                Eficiencia
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Capacidad de ahorro</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-border/40 shadow-lg animate-in fade-in slide-in-from-left-4 duration-500">
          <CardHeader>
            ...
            <CardDescription>Cómo se dividen tus gastos mensuales</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="w-full bg-muted rounded-full h-2 overflow-hidden">
                <div 
                  className={`h-full ${isPositive ? 'bg-green-600 dark:bg-green-400' : 'bg-red-600 dark:bg-red-400'} transition-all duration-500 rounded-full`}
        ...
        <Card className="border-border/40 shadow-lg animate-in fade-in slide-in-from-right-4 duration-500">
          <CardHeader>
            ...
            <CardDescription>Insights sobre tu salud financiera</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Estado actual */}
            <div className={`p-4 rounded-lg ${
              ...
              <div className="flex items-start gap-3">
                <estadoFinanciero.icon className={`h-5 w-5 mt-0.5 ${estadoFinanciero.color}`} />
                <div>
                  <h4 className="font-semibold text-sm mb-1">Estado: {estadoFinanciero.estado}</h4>
                  <p className="text-xs text-muted-foreground">
                    {balance > totalIngresos * 0.2 
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-muted/50 rounded-lg p-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {porcentajeGastosVariables > 30 ? '⚠️ Alto' : '✅ Controlado'}
                  </p>
                </div>
              </div>
