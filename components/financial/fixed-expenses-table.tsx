...
interface FixedExpensesTableProps {
  ...
  onAdd: (expense: Omit<FixedExpense, "id" | "createdAt">) => Promise<void>;
  onUpdate: (id: string, expense: Partial<FixedExpense>) => Promise<void>;
  onDelete: (id: string) => Promise<void>;
  currentMonth: string;
  currency: CurrencyCode;
}

export function FixedExpensesTable({ expenses, onAdd, onUpdate, onDelete, currentMonth, currency }: FixedExpensesTableProps) {
  ...
  const handleUpdate = async (id: string, field: keyof FixedExpense, value: string) => {
    await onUpdate(id, { [field]: value });
  };

  return (
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '100ms' }}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-1">
          <div className="h-10 w-10 rounded-xl bg-linear-to-br from-red-500 to-red-600 flex items-center justify-center shadow-lg shadow-red-500/20 animate-in zoom-in duration-300">
            <Receipt className="h-5 w-5 text-white" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-foreground">Gastos Fijos</h2>
            <p className="text-xs text-muted-foreground">Total: <span className="font-bold text-red-600 dark:text-red-400">
              {formatCurrency(
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button
              size="sm"
              className="bg-red-600 hover:bg-red-700 shadow-lg hover:shadow-xl transition-all hover:scale-105 animate-in zoom-in duration-300"
            >
